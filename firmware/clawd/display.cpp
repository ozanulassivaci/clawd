...
void Display::render(ClawdState state, int8_t usagePercent, NetworkStatus networkStatus) {
...
void Display::drawState(ClawdState state, int8_t usagePercent) {
  ...
  if (usagePercent >= 0) {
    char pctText[8];
    snprintf(pctText, sizeof(pctText), "%d%%", usagePercent);
    _u8g2.drawStr(DRAW_X_OFFSET + 2, DRAW_Y_OFFSET + 38, pctText);
  }
}
